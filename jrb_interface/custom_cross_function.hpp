...
// For implementation
template<template<bool> class Iface, int Id,class F1, class F2,class Derived>
struct custom_cross_function<Iface<true>,Id,F1,F2,Derived>{
private:
		portable_base* pV_;
public:

		enum{N = Iface<true>::base_sz + Id};
		std::function<F1> func_;
	typedef typename std::function<F1>::result_type ret;
	typedef typename fn_ptr_helper<F2>::fn_ptr_t vtable_fn_ptr_t;
		...
		void set_function(Func f){
			...
			*( (std::function<F1>*)vn->pFunctions_[N]) = f;
		}
...
protected:
	...
	struct helper{
		...
		std::function<F1>& get_function(){
			vtable_n_base* vn =(vtable_n_base*)pV_;

			return *( (std::function<F1>*)vn->pFunctions_[N]);
